...
def train_w2v(input_file, output_file, min_count_var, window_var, sg_cbow_var, size_var, iter_var, alpha_var, hsoft_negsamp_var, sample_var, negative_var, negexp_var): #,cbow_mean_var):
...
def eval_model(w2v_model, msl_dic):
    ...
    for key, value in msl_dic.items():
        ...
        if not ((" " in key[0]) or (" " in key[1])):
            curr_cos_similarity = w2v_model.wv.similarity(key[0], key[1])
        else:
            phrase1 = key[0].split()
            phrase2 = key[1].split()
            curr_cos_similarity = w2v_model.wv.n_similarity(phrase1, phrase2)
...
def eval_vectors(w2v_vectors, msl_dic):
    ...
    for key, value in msl_dic.items():
        try:
            w2v_vectors.get_vector(key[0]) 
            w2v_vectors.get_vector(key[1])
            spear_list_ann_scores.append(value)
            if not ((" " in key[0]) or (" " in key[1])):
                curr_cos_similarity = w2v_vectors.similarity(key[0], key[1])
            else:
                phrase1 = key[0].split()
                phrase2 = key[1].split()
                curr_cos_similarity = w2v_vectors.n_similarity(phrase1, phrase2)
            spear_list_cos_similarities.append(curr_cos_similarity)
        except KeyError:
            pass
...
def load_model():  
    load_m = gensim.models.word2vec.load('rmh_w2v.bin')    
    return load_m
